...
public class TypeChecker extends VisitorAdapter<Type> {
     ...
     * class or a field in an ancestor of the current class.
     *
    ...
    private Type getTypeForVar(Var v) {
        ...
        t = symbolTable.getFieldType(currentClassName, name);
        if (t != null) {
            return t;
        }
    ...
    private Type checkCall(List<String> tags, String cname, String mname, List<Exp> actuals) {
        String msgName = mname;
        if (cname != null) msgName = cname + "." + msgName;
        
        // check that target class does provide named method
        MethodSignature calledMethod = symbolTable.getMethodSignature(cname, mname);
