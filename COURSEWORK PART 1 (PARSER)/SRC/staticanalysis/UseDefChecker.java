...
public class UseDefChecker extends VisitorAdapter<Set<String>> {
    ...
    /**
     * Record in the definitions stack that local variable v has been
     * assigned a value.
     * This should only be called for a local variable which is currently
    ...
    // List<FieldDecl> fds;
    // List<MethodDecl> mds;
    public Set<String> visit(ClassDecl n) {
    ...
    // List<FieldDecl> fds;
    // List<MethodDecl> mds;
    public Set<String> visit(ClassDeclExtends n) {
        for (MethodDecl md : n.mds) {
            md.accept(this);
        }
        return null;
    ...
    public Set<String> visit(StmIf n) {
        ...
        Set<String> updates;
        if (eq(cond, TRUE)) {
            updates = n.st.accept(this);
        } else if (eq(cond, FALSE)) {
            updates = n.sf.accept(this);
        } else {
            Set<String> trueBranchUpdates = n.st.accept(this);
            undoDefs(trueBranchUpdates);
            if (blackHole) {
                blackHole = false;
                updates = n.sf.accept(this);
            } else {
                Set<String> falseBranchUpdates = n.sf.accept(this);
    ...
    public Set<String> visit(StmWhile n) {
        ...
        Set<String> updates;
        if (eq(cond, TRUE)) {
            updates = n.s.accept(this);
            blackHole = true;
        } else if (eq(cond, FALSE)) {
            updates = new HashSet<>();
        } else {
            Set<String> trueDefs = n.s.accept(this);
            undoDefs(trueDefs);
            updates = new HashSet<>();
        }
        return updates;
